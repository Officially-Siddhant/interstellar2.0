...
def force_gravity_world_nd(r_nd):
    R = np.linalg.norm(r_nd) # computes euclidean norm
    return - r_nd / (R**3)  # since mu~ = 1 and m~ = 1



# ----------------------------------------------------------------------
# Clohessy–Wiltshire / Hill-frame drift dynamics
# x_hill = [x, y, z]
# xd_hill = [xd, yd, zd]
#
# Used to compute:
#     f(x) = drift of the unforced Hill equations
#
# These are the equations your controller cancels using -f(x)
# ----------------------------------------------------------------------
def hill_drift(x_hill, xd_hill, n):
    """
    Compute the drift dynamics f(x) in the Hill frame.

    Inputs
    -------
    x_hill : (3,)  position in CW/Hill frame
    xd_hill: (3,)  velocity in CW/Hill frame
    n      : mean motion (rad/s)

    Returns
    -------
    f : (3,) drift accelerations from CW equations
    """

    x, y, z = x_hill
    xd, yd, zd = xd_hill

    # Clohessy–Wiltshire linearized equations:
    xdd = 3 * n**2 * x + 2 * n * yd
    ydd = -2 * n * xd
    zdd = -n**2 * z
...
# ----------------------------------------------------------------------
# Full Hill dynamics (time derivative of 6D state)
# X = [x, y, z, xd, yd, zd]
# u = control acceleration in Hill frame
# ----------------------------------------------------------------------
def hill_dynamics(X, u_hill, n):
    """
    Compute the full Hill-frame dynamics:
    ...
    with accelerations given by CW drift + control u.

    Inputs
    -------
    X       : (6,) state in Hill frame
    u_hill  : (3,) control acceleration in Hill frame
    n       : mean motion

    Returns
    -------
    Xdot : (6,) derivative of state in Hill frame
    """
    ...
    # drift term:
    f = hill_drift(np.array([x, y, z]),
                   np.array([xd, yd, zd]),
                   n)

    # total acceleration in Hill frame:
    a = f + u_hill
